...
var LT;
(function (LT) {
    function LigatureMain() {
        // LigatureMain.super(this)
        var label = new Laya.Label();
        label.text = "asdfasdfasdfasdfasdf"
        Laya.stage.addChild(label)
        var __prote = LigatureMain.prototype
 
        __prote.init = function(option){
            // 定义一共有多少行
        ...
        // 创建表格
        __prote.createTable = function(){
            var n=0;
            var tempW = this.canvas/this.imgRow;
            var tempH = this.canvas/this.imgCol;
            //遍历二维数组来创建行数和列数
            for(var i=0;i<this.dimenArr.length;i++){
                //创建tr 
                // var trObj = document.createElement('tr');
                var trObj = new Laya.Sprite()
                trObj.y = tempH * i;
                //添加到table中
                Laya.stage.addChild(trObj);
                for(var j=0;j<this.dimenArr[i].length;j++){
                    //创建td
                    // var tdObj = document.createElement('td');
                    var tdObj = new Laya.Sprite()
                    // 给表格创建大小
                    tdObj.width = tempW;
                    tdObj.height = tempH;
                    tdObj.x = tempW * j
                    //将td添加到tr中
                    trObj.addChild(tdObj);
                    ...
                    if(this.dimenArr[i][j]>0){
                        //改变img的src的值
                        imgObj.loadImage("comp/tab.png",0,0);
                        // imgObj.src = "images/"+this.dimenArr[i][j]+".jpg";
                        // 给每个img添加一个坐标索引
                        imgObj.indexX = i;
                        imgObj.indexY = j;
                        imgObj.on(laya.events.Event.CLICK,this, this.clickHandle,[imgObj,imgObj.indexX,imgObj.indexY]);
                    }
                }
            }
        },
        // 创建这个游戏
        __prote.createGame = function(){
        ...
        *
        */
        __prote.clickHandle = function(clickObj,x,y){
                ...
                if(obj.dimenArr[x][y]){			
                    ...
                    if(obj.temp == null){
                        ...
                        var path =  [
                            ...
                            //绘制矩形
                        obj.temp.parent.graphics.drawPath(20, 20, path, {fillStyle: "#ff0000"});
                        //将此时的行数和列数存入数组中
                        obj.pathInfo[0].x = x;
                        obj.pathInfo[0].y = y;

                    }else if(obj.temp != clickObj){  //两次点击的不是同一张图
                        ...
                        var path =  [
                            ...
                            //绘制矩形
                        obj.temp.parent.graphics.drawPath(20, 20, path, {fillStyle: "#000000"});
                        //将此时的行数和列数存入数组中
                        obj.pathInfo[1].x = x;
                        obj.pathInfo[1].y = y;
                        ...
                        if(obj.dimenArr[obj.pathInfo[0].x][obj.pathInfo[0].y] == obj.dimenArr[obj.pathInfo[1].x][obj.pathInfo[1].y] ){
                            ...
                            if(allPath){
                                ...
                                if(typeof allPath != "boolean"){
                                    ...
                                    //两张图片相邻就快速清除
                                    firstImg.parent.removeChild(firstImg);
                                    secondImg.parent.removeChild(secondImg);
                                }
        ...
        *
        */
        __prote.feasiblePath = function(A,B){  //A表示第一个点击的图片行列对象，B表示第二次点击的
            //如果两张图片相邻，直接消除
        ...
        //检查是否相邻
        __prote.checkAdjacent = function(A,B){
        ...
        * 
        */
        __prote.getPaths = function(element){  //获取点击图片的行数和列数
            //定义一个数组来存放十字线
            var paths = [];
            //遍历一共的行数
        ...
        // 判断两个点的连线上是否有图片
        __prote.checkTwoPath = function(target,current){
            //如果是同一列
        ...
        // 获取B点和有效点A同行同列的两个点
        __prote.getSamePostions = function(target,current){
            //定义一个数组存放同行同列的两个点的位置
        ...
        // 获取最短路径
        __prote.getShortPath = function(nearPath){
            var flag = true;
        ...
        // 获取路径的长度值，以便比较出最短路径
        __prote.getPathLength = function(currentCoords,targetCoords){
        ...
        // 获取交点绘线的方向
        __prote.direction = function(pathObj){
            var directionA = '';
            var directionB = '';
        ...
        __prote.pathDirection = function(current,target){
            // 如果两个点在同一行上，则在水平方向划线
            if(current.x == target.x){
                return "horizontal";
        ...
        // 格式化字符串
        __prote.formatStr = function(str){
            if(str == "Righttop"){
                return "topRight";
        ...
        // 得到路径上所有点坐标并画线
        __prote.pathCoordinate = function(pointAndCrossDirection){
            //判断绘制线的方向
            var pathDirection = ""; 
        ...
        // 获取所有的非相交点的路径点坐标
        __prote.getAllPoint = function(validPointA,validPointB){
            // 定义一个数组保存沿途经过的点
            var pointArr = [];
            // 如果两个点在同一行上
        ...
        // 画交点的线
        __prote.drawCrossDirection = function(path){
        ...
        __prote.drawPathLine = function(coordinateArr,direction){
            // 获取所有的tr对象
    ...
    LT.LigatureMain = LigatureMain;
    Laya.class(LigatureMain,'LT.LigatureMain');
})(LT || (LT = {}));
